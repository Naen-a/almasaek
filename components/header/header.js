...
export default function Header() {
  ...
  const goToProfilePage = () => {
    router.push(`/profile/${userState?.user?.iin}`);
  };
  ...
  const onExit = () => {
    const loading = toast.loading("Выход...");
      ...
      <div className="container mx-auto justify-between flex items-center">
        ...
        {userState?.user?.role === "student" && (
          <div className="flex items-center space-x-6 relative">
            ...
            <div
              className={`absolute top-12 flex flex-col -left-40 bg-white border-cobalt border px-5 py-2 w-64 ${menu && "hidden"}`}
            >
        ...
        {userState?.user?.role === "teacher" && (
          <div className="flex items-center space-x-6">
            ...
            >
              ...
              <span className="inline-block">Оценки</span>
            </button>
